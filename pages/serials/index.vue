...
export default {
  components: { movieCard },
	...
	computed : {
		...mapGetters( 
			{
				currentMoviesType:'serials/getCurrentMoviesType',
				movieCardIsloading:'serials/getMovieCardIsLoading'
			}
		),
	},
...
methods: {
    ...
    this.$store.dispatch('serials/axiosMovies', movieTypeIndex )
}
	...
	async fetch (context) {
		...
		for (let index = 0; index < context.store.state.movies.moviesTypes.length; index++) {
			await context.store.dispatch('serials/axiosMovies', [index] )
		}
