<template>
  <div class="">
    ...
    >
      <div
        class="my-bg rounded-lg overflow-hidden hidden xl:block"
        :style="`
    ...
    `"
      ...
      <div
        class="
          ...
          p-4
          w-full
          md:pl-8 md:justify-start md:items-start
        "
      >
        <h1
          :title="tv.name"
        ...
        <h2 class="text-lg font-bold mb-4">
          <span-ru-en ru=" О фильме :" en=" About Movie :" />
        </h2>
        ...
        <ul>
          ...
          <li class="mb-2">
            <span-ru-en class="font-medium" ru="Статус:" en="Status:" />
            {{ tv.status }}
          </li>
          <li class="mb-2" v-if="!!tv.belongs_to_collection">
            <span-ru-en
              class="font-medium"
              ru="Входит в коллекцию:"
              en="Collection:"
            /><NuxtLink
              class="border border-green bg-green-light p-1 rounded-md"
              :to="`/collection/${tv.belongs_to_collection.id}`"
              >{{ tv.belongs_to_collection.name }}</NuxtLink
            >
          </li>
        </ul>
      </div>
      <defalutImage
        :type="'movie'"
        :srcset="`https://image.tmdb.org//t/p/w400/${tv.poster_path}`"
    ...
    <div class="pt-8" v-if="tv.created_by">
      <h3 class="font-medium text-md">
        <span-ru-en ru="Cоздатели" en="Created By:" />
      </h3>
