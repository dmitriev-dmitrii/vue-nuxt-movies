...
<template>
  ...
  <form class="flex items-center relative  rounded-md  p-0 shadow w-full my-search-form" ref="searchForm" @submit.prevent="sendSearchReqest">
    ...
    <ul   class="ul-search-result absolute w-full  bg-white border border-gray border-t-0 rounded-bl-md rounded-br-md  shadow ">
      ...
      <li @click="unfocusForm" v-for="item in searchResultArray" :key=item.id :class=searchResultCardStyles> <NuxtLink    :to="`${routerLinkUrl(item)}`"  class="block w-full flex" >{{ item.title || item.name }} <span class="ml-auto block font-normal italic text-right">{{item.media_type}}</span> </NuxtLink></li>
    </ul>
